...
# class AudioNet_W2L(nn.Module):
#     def __init__(self, dim_aud = 76, win_size = 16):
#         super(AudioNet_W2L, self).__init__()
#         self.win_size = win_size
#         self.dim_aud = dim_aud
...
#             nn.LeakyReLU(0.02, True),
#             nn.Linear(in_features = 128, out_features = dim_aud)
#         )
...
# exp feature extractor
class ExpressionEncHalf(nn.Module):
    def __init__(self) -> None:
        super().__init__()
        self.encoder = nn.Sequential(
            nn.Linear(64, 32),
...
def get_half_signals(H, W, signals):
    sig_aud = signals[0]
    sig_eye = signals[1]
    signals_aud = torch.ones(H//2, W, sig_aud.shape[-1])
    signals_eye = torch.ones(H//2, W, sig_eye.shape[-1])
    signals_aud[:] = sig_aud
    signals_eye[:] = sig_eye
    sig_out = torch.cat([signals_eye, signals_aud], axis=0)
    
    return sig_out
    
# Ray helpers
def get_rays(H, W, focal, c2w, cx=None, cy=None, stride=1):
    # pytorch's meshgrid has indexing='ij'
    i, j = torch.meshgrid(torch.linspace(0, W-1, W//stride), torch.linspace(0, H-1, H//stride))
    i = i.t()
    j = j.t()
    if cx is None:
        cx = W*.5

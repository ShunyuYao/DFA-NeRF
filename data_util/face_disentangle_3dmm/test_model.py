...
from data_loader import Face3dmmDataset, FaceLSR2DatasetTest
import argparse
import os
...
def parse_args():
    ...
    parser.add_argument(
        "--test_path",
        type=str,
        default="logs/train_LSR2_data0725_dim32_16_202107251617",
        help="test path")
    parser.add_argument(
        "--save_path",
        type=str,
...
def test(dataloader, exp_dis_net, exp_cat_net, face_proj, lands_info):
    ...
    out_o = torch.cat(out_o_list)
    out_m = torch.cat(out_m_list)
...
def main():
    ...
    test_paths = [args.test_path]  # Mark_Zuck_old ysy_sjtu_talk JackMa_head
    test_dataset = Face3dmmDataset(test_paths)
    save_name = test_paths[-1].split("/")[-2]
    ...
    save_name = os.path.join(args.save_path, "{}_64_32.pt".format(save_name))

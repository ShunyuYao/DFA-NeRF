...
def train(epoch, args, dataloader, exp_dis_net, exp_cat_net, face_proj, 
          lands_info, optimizer_dis, optimizer_cat, scheduler_dis, scheduler_cat, loss_func):
    ...
    for exp_data in tqdm_switch_mouth:
        ...
        if not args.not_use_cycle_loss:
            ...
            loss_lan_back_exp_1 = loss_func(
                geometry_exp_back_1[:, :, :coord_dim], geometry_1[:, :, :coord_dim])
            loss_lan_back_exp_2 = loss_func(
            ...
            if args.use_exp_loss:
                loss_exp1 = loss_func(exp_para_1, exp_back_1)
                loss_exp2 = loss_func(exp_para_2, exp_back_2)
...
def test(epoch, args, save_path, dataloader, exp_dis_net, exp_cat_net, face_proj, lands_info, best_loss, loss_func):
...
def main():
    ...
    if args.resume_ckpt:
        exp_dis_net_path = os.path.join(args.resume_ckpt, 'dis_ckpt.pth')
        exp_cat_net_path = os.path.join(args.resume_ckpt, 'cat_ckpt.pth')
    ...
    optimizer_dis = optim.AdamW(exp_dis_net.parameters(), lr=TRAIN_LR)
    optimizer_cat = optim.AdamW(exp_cat_net.parameters(), lr=TRAIN_LR)
    ...
    # load data
    # train_paths = ['./face_3dmm_params/Mark_Zuck_old/', 
    # './face_3dmm_params/ysy_sjtu_talk/', 
    # './face_3dmm_params/obama/',
    # './face_3dmm_params/JackMa_head/'] # './face_3dmm_params/obama/'
    # test_paths = ['./face_3dmm_params/obama/']
    # train_dataset = Face3dmmDataset(train_paths)
    # test_dataset = Face3dmmDataset(test_paths)
    train_dataset = FaceLSR2Dataset('./dataset', 'train_0725')
    test_dataset = FaceLSR2Dataset('./dataset', 'val_0725')
    ...
    for epoch in range(start_epoch, train_epoch):
        ...
        with torch.no_grad():
            test_loss = test(epoch, args, log_path, test_dataloader, exp_dis_net, exp_cat_net, face_proj, 
                lands_info, best_loss, loss_func)

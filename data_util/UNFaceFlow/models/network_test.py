...
def write_mesh1(path, P, color, mask):
    fout=open(path, "w")
    # print(path)
    P = P[0, :, :]
    c = color[0, :, :]
    ...
    for i in range(mask.sum().item()):
        if mask[i]:
            fout.write("v " + str(P[i, 0].item()) + " " + str(P[i, 1].item()) + " " + str(P[i, 2].item()) + " " + str(c[i, 0].item()) + " " + str(c[i, 1].item()) + " " + str(c[i, 2].item()) + "\n")
    fout.close()
    return
...
class NeuralNRT(nn.Module):
    ...
    def construct_corres(self,src_input,tar_input,src_crop_im,tar_crop_im,src_mask,tar_mask,src_parsing_mask,tar_parsing_mask,src_Vindex, src_Valpha, src_pts, tar_pts, \
        ...
        feature_fw = torch.zeros((N, feature_fw_crop.shape[1], self.opt.height, self.opt.width), device=grid.device)
        feature_bw = torch.zeros((N, feature_bw_crop.shape[1], self.opt.height, self.opt.width), device=grid.device)
        ...
        for j in range(len(flow_bw_crop)):
            ...
            for i in range(N):
                ...
                if j==((len(flow_bw_crop))-1):
                    feature_fw_cropi = F.interpolate(feature_fw_crop[i:(i+1)], ((Crop_param[i, 3, 0]-Crop_param[i, 2, 0]).item(), (Crop_param[i, 1, 0]-Crop_param[i, 0, 0]).item()), mode='bilinear')
                    feature_bw_cropi = F.interpolate(feature_bw_crop[i:(i+1)], ((Crop_param[i, 7, 0]-Crop_param[i, 6, 0]).item(), (Crop_param[i, 5, 0]-Crop_param[i, 4, 0]).item()), mode='bilinear')
                    feature_fw[i, :, Crop_param[i, 2, 0]:Crop_param[i, 3, 0], Crop_param[i, 0, 0]:Crop_param[i, 1, 0]] = feature_fw_cropi[0]
                    feature_bw[i, :, Crop_param[i, 6, 0]:Crop_param[i, 7, 0], Crop_param[i, 4, 0]:Crop_param[i, 5, 0]] = feature_bw_cropi[0]
...
class LossClass(nn.Module):
    ...
    def loss_on_lfd(self, deformation_p, orig_color, p1, pts, tgt_color, weights, orig_bool, tgt_bool, tgt_pts_bool, mean_img):
        ...
        write_mesh1("./debug/tar.obj", p1, tgt_color, tgt_bool)
        write_mesh1("./debug/warp_raw.obj", deformation_p, orig_color, orig_bool)
        ...
        write_mesh1("./debug/warp1.obj", deformation_p, orig_color, orig_bool)
        exit(0)
        for view_id in range(self.view_divide**2):
    ...
    def forward(self, flow_lists, flow_bw_lists, src_input, tar_input, src_mask, weights, weights_bw, fw_outrange_mask, bw_outrange_mask, tar_mask, Q_Fill, src_Fill_im, src_bool, weights_bw_Fill, \
        ...
        write_mesh1("./debug/src.obj", P_Fill, src_Fill_im, src_bool)

...
class NeuralNRT(nn.Module):
    ...
    def forward(self, src_input, tar_input, src_crop_im, tar_crop_im, Crop_param, src_mask, tar_mask, src_parsing_mask, tar_parsing_mask, src_pts, tar_pts, src_recon_pts, tar_recon_pts):

        flow_lists, flow_bw_lists, fw_outrange_mask, bw_outrange_mask=self.construct_corres(src_input,tar_input,src_crop_im,tar_crop_im,src_mask,tar_mask, Crop_param)

        loss_ph, loss_smooth, loss_parsing, loss_pts = self.LossClass(flow_lists, flow_bw_lists, src_input, tar_input, src_mask, \
            fw_outrange_mask, bw_outrange_mask, tar_mask, tar_parsing_mask, src_parsing_mask, src_pts, tar_pts, src_recon_pts, tar_recon_pts)
        return loss_smooth.unsqueeze(0), loss_parsing.unsqueeze(0), loss_ph.unsqueeze(0), loss_pts.unsqueeze(0)
...
class LossClass(nn.Module):
    ...
    def pts_loss(self, corres, src_pts, src_recon_pts, tar_pts, tar_recon_pts, src_mask):
        N = corres.shape[0]
        ...
        Bs = torch.arange(N, device=src_pts.device).view(-1, 1).repeat(1, self.opt.num_pts).view(-1)
        ...
        zeros_ = torch.zeros((N*self.opt.num_pts), device=src_pts.device, dtype=torch.long)[valid_pts_mask]
        ones_ = torch.ones((N*self.opt.num_pts), device=src_pts.device, dtype=torch.long)[valid_pts_mask]

        Bs_recon = torch.arange(N, device=src_pts.device).view(-1, 1).repeat(1, self.opt.num_recon_pts).view(-1)
        ...
        zeros_recon = torch.zeros((N*self.opt.num_recon_pts), device=src_pts.device, dtype=torch.long)[valid_recon_pts_mask]
        ones_recon = torch.ones((N*self.opt.num_recon_pts), device=src_pts.device, dtype=torch.long)[valid_recon_pts_mask]

        corres = corres.view(N, 2, -1).permute(0, 2, 1)
        # print(corres_clone.max(), corres_clone.min(), tar_pts.max(), tar_pts.min())
        x_diff = corres[(Bs, src_pts_pos, zeros_)] - tar_pts[:, :, 1].view(-1)[valid_pts_mask].float()
        y_diff = corres[(Bs, src_pts_pos, ones_)] - tar_pts[:, :, 0].view(-1)[valid_pts_mask].float()
        x_recon_diff = corres[(Bs_recon, src_recon_pts_pos, zeros_recon)] - tar_recon_pts[:, :, 1].view(-1)[valid_recon_pts_mask].float()
        y_recon_diff = corres[(Bs_recon, src_recon_pts_pos, ones_recon)] - tar_recon_pts[:, :, 0].view(-1)[valid_recon_pts_mask].float()
        loss = (x_diff**2 + y_diff**2).mean() + (x_recon_diff**2 + y_recon_diff**2).mean()
        return loss

    def forward(self, flow_lists, flow_bw_lists, src_input, tar_input, src_mask, \
            fw_outrange_mask, bw_outrange_mask, tar_mask, tar_parsing_mask, src_parsing_mask, src_pts, tar_pts, src_recon_pts, tar_recon_pts):
        N = src_input.shape[0]
        ...
        for i in range(n_predictions):
            ...
            loss_parsing_fw = self.parsing_loss(src_parsing_mask, new_tar_parsing_mask, src_mask, fw_outrange_mask)
            loss_parsing_bw = self.parsing_loss(tar_parsing_mask, new_src_parsing_mask, tar_mask, bw_outrange_mask)
            ...
            loss_pts_fw = self.pts_loss(corres, src_pts, src_recon_pts, tar_pts, tar_recon_pts, src_mask)
            loss_pts_bw = self.pts_loss(corres_bw, tar_pts, tar_recon_pts, src_pts, src_recon_pts, tar_mask)
            loss_pts += i_weight * (loss_pts_bw + loss_pts_fw) / 2.0
        ...
        return loss_ph, loss_smooth, loss_parsing, loss_pts

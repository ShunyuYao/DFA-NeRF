...
class FaceMobilenet(nn.Module):
    ...
    def __init__(self, cfg, is_train=False, progress=True, **kwargs):
        ...
        self.use_head_pose = extra.USE_HEAD_POSE if "USE_HEAD_POSE" in extra else False
        ...
        if self.use_head_pose:
            self.head_pose_conv = nn.Sequential(
                ConvBNReLU(self.c3_last_channel, 128),
    ...
    def forward(self, x):
        ...
        if self.use_regress:
            ...
            if self.use_head_pose:
                x_hp = self.head_pose_conv(c3)
                x_hp = x_hp.mean([2, 3])
                x_hp = self.head_pose_fc(x_hp)
                outputs['head_pose'] = x_hp
            if self.dense_regression:

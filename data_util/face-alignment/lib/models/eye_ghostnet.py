...
def depthwise_conv(inp, oup, kernel_size=3, stride=1, dilation=1, relu=False):
    return nn.Sequential(
        nn.Conv2d(inp, oup, kernel_size, stride, kernel_size//2, groups=inp, bias=False, dilation=dilation),
        nn.BatchNorm2d(oup),
...
class GhostBottleneck(nn.Module):
    def __init__(self, inp, hidden_dim, oup, kernel_size, stride, use_se, dilation=1):
        ...
        self.conv = nn.Sequential(
            ...
            # dw
            depthwise_conv(hidden_dim, hidden_dim, kernel_size, stride, dilation=dilation, relu=False) if stride==2 else nn.Sequential(),
            # Squeeze-and-Excite
...
class GhostNet(nn.Module):
    def __init__(self, sets, cfg, is_train=False, width_mult=1.):
        ...
        for k, exp_size, c, use_se, s in self.sets:
...
def get_eye_net(cfg, is_train=False, progress=True, **kwargs):
    ...
    #     [5, 128,  64, 1, 1],
    # ]
    model = GhostNet(sets, cfg, is_train, **kwargs)

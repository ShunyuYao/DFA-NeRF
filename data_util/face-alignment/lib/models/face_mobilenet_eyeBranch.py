...
class FaceMobilenet(nn.Module):
    ...
    def __init__(self, cfg, is_train=False, progress=True, **kwargs):
        ...
        self.c3_last_channel = 64
        self.c4_last_channel = 160
        ...
        if self.use_regress:
            ...
            if self.dense_regression:
                ...
                # self.s3_eye_layer = nn.Sequential(
                #     nn.Linear(self.c3_last_channel, cfg.MODEL.NUM_EYE_JOINTS * 4),
                #     )
                self.s4_conv_layer = nn.Sequential(
                    ConvBNReLU(self.c4_last_channel, 128),
                    ConvBNReLU(128, 128, groups=8),
                    ConvBNReLU(128, 128, groups=8),
                    )
                self.s4_fc_layers = nn.Sequential(
                    nn.Dropout(0.2),
                    nn.Linear(128, cfg.MODEL.NUM_EYE_JOINTS * 4),
                    )
    ...
    def forward(self, x):
        ...
        if self.use_regress:
            ...
            if self.dense_regression:
                ...
                # x_s3 = self.s3_eye_layer(x1)
                x_s4 = self.s4_conv_layer(x2)
                x_s4 = x_s4.mean([2, 3])
                x_s4 = self.s4_fc_layers(x_s4)
                x_fc = self.fc_layers(x3)
            if self.lr_range_test:
                return x_fc
            # outputs["s3_regress"] = x_s3
            outputs["s4_regress"] = x_s4

    ...
    def __init__(self, cfg, is_train=False, progress=True, **kwargs):
        ...
        if self.use_regress:
            ...
            self.fc_layers = nn.Sequential(
                nn.Dropout(0.2),
                nn.Linear(64 * self.output_feats_size ** 2, 512),
                nn.ReLU(inplace=True),
                nn.Dropout(0.2),
                nn.Linear(512, 512),
                nn.ReLU(inplace=True),
                nn.Linear(512, cfg.MODEL.NUM_FACE_JOINTS*2)
                )
...
def get_pose_net(cfg, is_train=False, progress=True, **kwargs):
    """
    ...
    """
    model = PoseMobilenet(cfg, is_train, progress, **kwargs)

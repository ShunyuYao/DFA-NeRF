...
def cal_grad_loss(tensor_list, weight_list):
    grad_kernel = torch.Tensor(
        (-1.0, 1.0)).unsqueeze(0).unsqueeze(0).float().to(tensor_list[0].device)
    loss_grad = 0
    for i in range(len(tensor_list)):
        in_tensor = tensor_list[i]
        in_tensor = in_tensor.view(-1, 1, in_tensor.shape[-1])
        out_tensor = F.conv1d(in_tensor, grad_kernel)
        loss_grad += torch.mean(out_tensor**2)*weight_list[i]
    return loss_grad
